...
std::vector<Plaintext> Decryptor::Decrypt(ConstSpan<Ciphertext> cts) const {
    ...
    std::shared_ptr<char> src_pen(fpga_engine::c_malloc_init_zero(key_conf_.cipher_byte_ * cts_size), lambda_deleter);
    std::shared_ptr<char> src_base(fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * cts_size), lambda_deleter);
    std::shared_ptr<char> src_exp(fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * cts_size), lambda_deleter);
    ...
    for (auto item : cts) {
        memcpy(src_pen_ptr + i * key_conf_.cipher_byte_, item->GetMantissa(), key_conf_.cipher_byte_);

        memcpy(src_base_ptr + i * CFPGATypes::U_INT32_BYTE, &cipher_base, CFPGATypes::U_INT32_BYTE);
        
        int cur_exp = item->GetExp();
        memcpy(src_exp_ptr + i * CFPGATypes::U_INT32_BYTE, &cur_exp, CFPGATypes::U_INT32_BYTE);

        i++;
    }

    // Part 2 Decrypt
    std::shared_ptr<char> res_fpn(fpga_engine::c_malloc_init_zero(key_conf_.plain_byte_ * cts_size), lambda_deleter);
    std::shared_ptr<char> res_base(fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * cts_size), lambda_deleter);
    std::shared_ptr<char> res_exp(fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * cts_size), lambda_deleter);
    ...
    fpga_engine::decrypt(src_pen.get(), src_base.get(), src_exp.get(),
                         res_fpn.get(), res_base.get(), res_exp.get(),
                         pub_key_n, nullptr, nullptr, nullptr,
                         pri_key_p, pri_key_q, pri_key_p_square, pri_key_q_square,
                         pri_key_q_inverse, pri_key_hp, pri_key_hq,
                         key_conf_.plain_bits_, key_conf_.cipher_bits_, 
                         cts_size, fpga_dev_num);
...
std::vector<Plaintext> Decryptor::Decode(std::shared_ptr<char>& res_fpn,
    ...
    std::shared_ptr<char> res_sptr(fpga_engine::c_malloc_init_zero(CFPGATypes::INT64_BYTE * cts_size), lambda_deleter);
    char *pub_key_n = pub_key_helper_.GetBytesN();
    char *pub_key_max_int = pub_key_helper_.GetBytesMaxInt();
    fpga_engine::decode_int(
        res_fpn.get(), res_base.get(), res_exp.get(),
        pub_key_n, pub_key_max_int,
        key_conf_.plain_bits_,
        res_sptr.get(),
        cts_size
    );
