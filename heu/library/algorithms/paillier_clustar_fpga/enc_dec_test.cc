...
using heu::lib::algorithms::paillier_clustar_fpga::fpga_engine::
    c_malloc_init_zero;
...
INSTANTIATE_TEST_SUITE_P(SubTest, CEncDecTest, ::testing::Values(1024));
...
TEST_P(CEncDecTest, EncodeDecode) {
  ...
  std::vector<Plaintext> res_plain_vec =
      decryptor_->Decode(res_fpn, res_base_fpn, res_exp_fpn, pts_size);

  // Compare
  for (size_t i = 0; i < vec_size; i++) {
    // std::cout<<"res_plain_vec["<<i<<"]:
    // "<<res_plain_vec[i].Get<int64_t>()<<std::endl;
...
TEST_P(CEncDecTest, EncryptZero) {
  ...
  auto enc_zeros_span =
      absl::MakeConstSpan(enc_zeros_ptrs.data(), enc_zeros_ptrs.size());
  ...
  auto dec_zeros_span =
      absl::MakeSpan(dec_zeros_ptrs.data(), dec_zeros_ptrs.size());
...
TEST_P(CEncDecTest, EncryptMaxNum) {
  ...
  uint64_t overflow_val =
      static_cast<uint64_t>(std::numeric_limits<int64_t>::max()) + 10;
  std::vector<uint64_t> a_vec{234,  890, overflow_val, 0, overflow_val - 1,
                              10001};
  std::vector<Plaintext> a_plain_vec;
  auto vec_size = a_vec.size();
  for (size_t i = 0; i < vec_size; i++) {
