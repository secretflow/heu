...
const char DEV_C2H[MAX_NUM_OF_DEV][4][20] = {
	{"/dev/xdma0_c2h_0", "/dev/xdma0_c2h_1", "/dev/xdma0_c2h_2", "/dev/xdma0_c2h_3"},
	{"/dev/xdma1_c2h_0", "/dev/xdma1_c2h_1", "/dev/xdma1_c2h_2", "/dev/xdma1_c2h_3"},
	{"/dev/xdma2_c2h_0", "/dev/xdma2_c2h_1", "/dev/xdma2_c2h_2", "/dev/xdma2_c2h_3"},
	{"/dev/xdma3_c2h_0", "/dev/xdma3_c2h_1", "/dev/xdma3_c2h_2", "/dev/xdma3_c2h_3"},
	{"/dev/xdma4_c2h_0", "/dev/xdma4_c2h_1", "/dev/xdma4_c2h_2", "/dev/xdma4_c2h_3"},
	{"/dev/xdma5_c2h_0", "/dev/xdma5_c2h_1", "/dev/xdma5_c2h_2", "/dev/xdma5_c2h_3"},
	{"/dev/xdma6_c2h_0", "/dev/xdma6_c2h_1", "/dev/xdma6_c2h_2", "/dev/xdma6_c2h_3"},
	{"/dev/xdma7_c2h_0", "/dev/xdma7_c2h_1", "/dev/xdma7_c2h_2", "/dev/xdma7_c2h_3"}
};
const char DEV_H2C[MAX_NUM_OF_DEV][4][20] = {
	{"/dev/xdma0_h2c_0", "/dev/xdma0_h2c_1", "/dev/xdma0_h2c_2", "/dev/xdma0_h2c_3"},
	{"/dev/xdma1_h2c_0", "/dev/xdma1_h2c_1", "/dev/xdma1_h2c_2", "/dev/xdma1_h2c_3"},
	{"/dev/xdma2_h2c_0", "/dev/xdma2_h2c_1", "/dev/xdma2_h2c_2", "/dev/xdma2_h2c_3"},
	{"/dev/xdma3_h2c_0", "/dev/xdma3_h2c_1", "/dev/xdma3_h2c_2", "/dev/xdma3_h2c_3"},
	{"/dev/xdma4_h2c_0", "/dev/xdma4_h2c_1", "/dev/xdma4_h2c_2", "/dev/xdma4_h2c_3"},
	{"/dev/xdma5_h2c_0", "/dev/xdma5_h2c_1", "/dev/xdma5_h2c_2", "/dev/xdma5_h2c_3"},
	{"/dev/xdma6_h2c_0", "/dev/xdma6_h2c_1", "/dev/xdma6_h2c_2", "/dev/xdma6_h2c_3"},
	{"/dev/xdma7_h2c_0", "/dev/xdma7_h2c_1", "/dev/xdma7_h2c_2", "/dev/xdma7_h2c_3"}
};

uint8_t get_data_width(size_t n_length) {
	switch(n_length) {
		case 128:  return 0x0; break;
		case 256:  return 0x1; break;
		case 512:  return 0x2; break;
		case 768:  return 0x3; break;
		case 1024: return 0x4; break;
		case 2048: return 0x5; break;
		case 3072: return 0x6; break;
		case 4096: return 0x7; break;
...
int close_dev(int userfd, int h2cfd, int c2hfd, uint8_t dev_num) {
	...
	rc = close(user_fd);
	if (rc != 0) {
		fprintf(stderr, "unable to close dev %s, %d.\n", DEV_USER[fpga_id], user_fd);
		return ERROR_CLOSE_DRIVER;
	}
	rc = close(h2c_fd);
	if (rc != 0) {
		fprintf(stderr, "unable to close dev %s, %d.\n", DEV_C2H[fpga_id][0], h2c_fd);
		return ERROR_CLOSE_DRIVER;
	}
	rc = close(c2h_fd);
	if (rc != 0) {
		fprintf(stderr, "unable to close dev %s, %d.\n", DEV_H2C[fpga_id][0], c2h_fd);
		return ERROR_CLOSE_DRIVER;
	}
...
int fpga_dev_number_get() {
	...
	for(uint32_t i = 0; i < MAX_NUM_OF_DEV; i++) {	
		...
		char *map_base = static_cast<char*>(mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, user_fd, 0));
		if (map_base == (void *)-1) {
 ...
 * Description : used to achieve Modexp_Multifunc accleration in FPGA, including handshake between FPGA and HOST,
 *               register configuration, data transfer, Modexp_Multifunc accleration, etc.
 * Para:
 *   cfg       : Configuration Information
 *   para      : ModexpMultifunc's para to FPGA
 *   data1     : ModexpMultifunc's data1 to FPGA
 *   data2     : ModexpMultifunc's data1 to FPGA
 ...
 *   other     : FAILURE, refer to Error Id.
 */ 
int fpga_fedai_operator_accl(fpga_config *cfg, char *para, char *data1, char *data2, char *data3, char *result){
	#ifdef TASK_DEBUG
		printf("task id inside are %d, %d, %d\n",cfg->data1_memtype, cfg->data2_memtype, cfg->data3_memtype);
	#endif
	long long rc;
	// uint32_t para_length, data1_length, data2_length, data3_length;
	// uint32_t dev_num = cfg->fpga_dev_id >= 0 ? cfg->fpga_dev_id : 255; // Ooops: fpga_dev_id(uint8_t) always >= 0
	...
	if (cfg->para_bitlen == 0x8) {
		fprintf(stderr, "***** invalid data0_length, only supports 0, 128, 256, 512, 768, 1024, 2048, 3072, 4096\n");
		return ERROR_LENGTH;
	}
	else if(cfg->data1_bitlen == 0x8) {
		fprintf(stderr, "***** invalid data1_length, only supports 0, 128, 256, 512, 768, 1024, 2048, 3072, 4096\n");
		return ERROR_LENGTH;
	}
	else if(cfg->data2_bitlen == 0x8) {
		fprintf(stderr, "***** invalid data2_length, only supports 0, 128, 256, 512, 768, 1024, 2048, 3072, 4096\n");
		return ERROR_LENGTH;
	}
	else if(cfg->data3_bitlen == 0x8) {
		fprintf(stderr, "***** invalid data3_length, only supports 0, 128, 256, 512, 768, 1024, 2048, 3072, 4096\n");
		return ERROR_LENGTH;
	}
	...
	if ((cfg->result_memtype == 0 && result == NULL) \
	 || (cfg->result_memtype != 0 && (cfg->result_memtype < 128 || cfg->result_memtype > (128 + 15))) \
	 || (cfg->result_memtype != 0 && (cfg->result_memtype == cfg->data1_memtype \
	 || cfg->result_memtype == cfg->data1_memtype || cfg->result_memtype == cfg->data3_memtype))){
	...
	switch(cfg->operate_mode) {
		...
		case 15:
		case 16:
			if (cfg->data1_memtype == 255 || (cfg->data1_memtype == 0 && data1 == NULL) \
		...
		case 12:
			if (cfg->data1_memtype == 255 || (cfg->data1_memtype == 0 && data1 == NULL) \
			 || cfg->data2_memtype == 255 || (cfg->data2_memtype == 0 && data2 == NULL) \
		...
		case 5:
			if (cfg->data1_memtype == 255 || (cfg->data1_memtype == 0 && data1 == NULL) \
			 || cfg->data3_memtype == 255 || (cfg->data3_memtype == 0 && data3 == NULL) \
		...
		case 6:
			...
			break;
		case 11:
			if (cfg->data2_memtype == 255 || (cfg->data2_memtype == 0 && data2 == NULL) \
	...
	if(rc != 0) {
		fprintf(stderr, "***** Memtypes of Input data do not match operate mode.\n");
		return rc;
	}
	...
	uint32_t task_launch_flag, srcdata_vld_flag, backdata_vld_flag, result_buffer_flag;
	...
	if(dev_num>=128 && dev_num <=135)
	{
		...
		if(fpga_id != 0)
		{
			rc = open_dev(&userfd[fpga_id], &h2cfd[fpga_id], &c2hfd[fpga_id], fpga_id);
			map_base = static_cast<char*>(mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, userfd[fpga_id], 0));
			user_fd = userfd[fpga_id];
			h2c_fd = h2cfd[fpga_id];
			c2h_fd = c2hfd[fpga_id];
		}
		else
			map_base = static_cast<char*>(mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, userfd[0], 0));
		#ifdef MULTI_CARD_DEBUG
	...
	else
	{
		...
		task = (struct fpga_modexp_task *)malloc(sizeof(struct fpga_modexp_task));
		if((cfg->data1_memtype <= (128 + 15) && cfg->data1_memtype >= 128) || (cfg->data2_memtype <= (128 + 15) && cfg->data2_memtype >= 128) || (cfg->data3_memtype <= (128 + 15) && cfg->data3_memtype >= 128) || (cfg->result_memtype <= (128 + 15) && cfg->result_memtype >= 128))
		{
			...
			// Check whether the input numbers are buffered in FPGA and whether their information matches cfg
			...
			for (i = 0; i < 3; i++) {
				if (memtype_check[i] <= (128 + 15) && memtype_check[i] >= 128) {
					...
					if (backdata_vld_flag != 1 || result_buffer_flag != 1 || task_launch_flag == 1 || srcdata_vld_flag == 1){
						rc = ERROR_SRCDAT_BUFSTATE;
						fprintf(stderr, "***** No input data buffered in the given task id.\n");
					...
					addr_base = DDR1_BASE_ADDR + (memtype_check[i] - 128) * 1024 * 1024 * 1024llu;
					bit_width_check = 0;
					batch_size_check = 0;

					flock(c2hfd[0], LOCK_EX);
					transfer_from_fpga(DEV_C2H[fpga_id][0], c2hfd[0], buffer_data_info, BACKDATA_INFO_SIZE, addr_base);
					...
					if(bit_width_check != bitlen_check[i] || batch_size_check != cfg->batch_size){
						rc = ERROR_SRCDAT_BUFINFO;
						fprintf(stderr, "***** Information buffered in FPGA contradicts cfg of input data%d.\n", i + 1);
		...
		else
		{
			...
			{
				map_base = static_cast<char*>(mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, userfd[i], 0));
				if (map_base == (void *)-1) {
			...
			else if(matched_deviceid_count == 1)
			{
				...
				if(dev_task_busy_count<dev_taskid_count)
				{
					...
					{
						if(k != 0)
							close_dev(userfd[k], h2cfd[k], c2hfd[k], k);
					}
					map_base = static_cast<char*>(mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, userfd[fpga_id], 0));
					dev_num = fpga_id;
					user_fd = userfd[fpga_id];
				...
				{	
					...
					if(min_busy_counts==taskid_counts[matched_deviceid[0]])
					{
						...
						do{
							for(i=0;i<matched_deviceid_count;i++)
							{
								map_base = static_cast<char*>(mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, userfd[matched_deviceid[i]], 0));
								uint32_t dev_task_busy_count = 0;
								task_busy_count = map_base + TASK_BUSY_COUNT_ADDR;
								dev_task_busy_count = reg32_read(task_busy_count);
			...
			max_task_memspace = memory_total_size[fpga_id] / taskid_counts[fpga_id] * 1024 * 1024 *1024 - 16 * 1024;
			//fprintf(stdout,"cmd_size %ld, para_size %ld, data_size %ld, total_size %ld, max_task_space %ld\n",16*1024,cfg->para_data_size, cfg->data1_size,16*1024+cfg->para_data_size+cfg->data1_size,max_task_memspace+16*1024);
			if ((cfg->para_data_size + cfg->data1_size + cfg->data2_size + cfg->data3_size) > (max_task_memspace)) {                             /////////!!!!!!!!MAX_MEMSPACE
				fprintf(stderr, "***** Input data size is larger than maximum storage size.\n");
	...
	task_id = 0;
	if (cfg->result_memtype == 0){ // Poll to distribute a task id for current work
		poll_num = 0;
		do{
			...
			backdata_vld_flag = get_status(task->backdata_state_addr, 0);
			result_buffer_flag = get_status(task->result_buffer_state_addr, 0);
			if(task_launch_flag || srcdata_vld_flag || backdata_vld_flag || result_buffer_flag) {
				usleep(50);
	...
	else { // Task id is already distributed, just check the status of registers of this task id 
		...
		backdata_vld_flag = get_status(task->backdata_state_addr, 0);
		result_buffer_flag = get_status(task->result_buffer_state_addr, 0);
		if (task_launch_flag == 1 || srcdata_vld_flag == 1 || backdata_vld_flag == 1 || result_buffer_flag == 0){
	...
	// uint64_t data_base_offset = para_data_base + cfg->para_data_size;
	uint64_t data1_base = data_base_offset, data2_base = data_base_offset, data3_base = data_base_offset;
	if (cfg->data1_memtype == 0){
		...
		data1_base = DDR1_BASE_ADDR + BACKDATA_INFO_SIZE + (cfg->data1_memtype - 128) * 1024 * 1024 * 1024llu;
	}
	if (data_base_offset % 64 != 0){
		data_base_offset = (data_base_offset / 64 + 1) * 64;
	}
	...
	if (cfg->data2_memtype == 0){
		...
		data2_base = DDR1_BASE_ADDR + BACKDATA_INFO_SIZE + (cfg->data2_memtype - 128) * 1024 * 1024 * 1024llu;
	}
	if (data_base_offset % 64 != 0){
		data_base_offset = (data_base_offset / 64 + 1) * 64;
	}
	...
	if (cfg->data3_memtype == 0){
		...
		data3_base = DDR1_BASE_ADDR + BACKDATA_INFO_SIZE + (cfg->data3_memtype - 128) * 1024 * 1024 * 1024llu;
	}
	...
	if (cfg->data1_memtype <= (128 + 15) && cfg->data1_memtype >= 128) src_buffer_flag = src_buffer_flag | 1;
	if (cfg->data2_memtype <= (128 + 15) && cfg->data2_memtype >= 128) src_buffer_flag = src_buffer_flag | 2;
	if (cfg->data3_memtype <= (128 + 15) && cfg->data3_memtype >= 128) src_buffer_flag = src_buffer_flag | 4;
...
#endif
	uint32_t task_config = (0x0 << 28) | ((cfg->data3_bitlen & 0xF) << 24) | ((cfg->data2_bitlen & 0xF) << 20) | \
						   ((cfg->data1_bitlen & 0xF) << 16) | ((cfg->para_bitlen & 0xF) << 12) | \
	...
	rc = posix_memalign((void **)&cmd, 4096, (sizeof(char) * cmd_data_size) + 4096);
	if (rc != 0) {
		flock(h2cfd[0], LOCK_UN);
		free_task(task);
		if(fpga_id != 0)
...
#endif
#ifdef DEBUG
	fprintf(stdout, "*****fpga_id %d CPU Transfer task cmd  to FPGA (Addr: 0x%08lx, Data size: %d bytes).\n", fpga_id,cmd_data_base, cmd_data_size);
#endif
#ifdef MULTI_CARD_DEBUG
...
#endif
	rc = 0;
	rc = transfer_to_fpga(DEV_H2C[fpga_id][0], h2c_fd, cmd, cmd_data_size, cmd_data_base);
	if (rc < 0) {
...
#ifdef DEBUG
	fprintf(stdout, "***** CPU Transfer task para to FPGA (Addr: 0x%08lx, Data size: %zu bytes).\n", para_data_base,  cfg->para_data_size);
#endif
	rc = 0;
	rc = transfer_to_fpga(DEV_H2C[fpga_id][0], h2c_fd, para, cfg->para_data_size, para_data_base);
	if (rc < 0) {
	...
	if (cfg->data1_memtype == 0) {
#ifdef DEBUG	
	fprintf(stdout, "***** CPU Transfer task data to FPGA (Addr: 0x%08lx, Data size: %zu bytes).\n", data1_base, cfg->data1_size);
#endif
		rc = 0;
		rc = transfer_to_fpga(DEV_H2C[fpga_id][0], h2c_fd, data1, cfg->data1_size, data1_base);
		if (rc < 0) {
			fprintf(stderr, "***** ERROR happend during transfering data1 to FPGA.\n");
			free_task(task);
			flock(h2cfd[0], LOCK_UN);
			if(fpga_id != 0)
			{	
...
#endif
		rc = 0;
		rc = transfer_to_fpga(DEV_H2C[fpga_id][0], h2c_fd, data2, cfg->data2_size, data2_base);
		if (rc < 0) {
			fprintf(stderr, "***** ERROR happend during transfering data2 to FPGA.\n");
			free_task(task);
			flock(h2cfd[0], LOCK_UN);
			if(fpga_id != 0)
			{	
...
#endif
		rc = 0;
		rc = transfer_to_fpga(DEV_H2C[fpga_id][0], h2c_fd, data3, cfg->data3_size, data3_base);
		if (rc < 0) {
			fprintf(stderr, "***** ERROR happend during transfering data3 to FPGA.\n");
			free_task(task);
			flock(h2cfd[0], LOCK_UN);
			if(fpga_id != 0)
			{	
	...
	// long double time_after = (fpgaid_time_point[fpga_id][6].tv_sec) * 1000.0 + (fpgaid_time_point[fpga_id][6].tv_usec) / 1000.0;
	//fprintf(stdout,"fpga_id %d, After Send Time %16.4Lf (ms)\n",fpga_id,time_after);
	gettimeofday(&time_point[6], NULL);
	// long double time_after_send = (time_point[6].tv_sec) * 1000.0 + (time_point[6].tv_usec) / 1000.0;
	...
	do{
		...
		else{ // in case that FPGA poll down srcdata_vld_flag because of wrong cfg info.
			task_status = reg32_read(task->task_state_addr);
			task_error_id = (task_status >> 8) & 0xFF;
			if (task_error_id != 0) {
				fprintf(stdout, "Detected task_error_id %d\n", task_error_id);
	...
	rc = 0;
	do{
		if(poll_num * 50 > single_task_maximum_time){ //todo : confirm this value 
...
#ifdef DEBUG
	fprintf(stdout, "***** FPGA's task done! FPGA has finished the Algorithm accl.\n");
#endif

	//debugcxd
	// printf("9\n");
	gettimeofday(&fpgaid_time_point[fpga_id][7], NULL);
	// long double time_before = (fpgaid_time_point[fpga_id][7].tv_sec) * 1000.0 + (fpgaid_time_point[fpga_id][7].tv_usec) / 1000.0;
	//fprintf(stdout,"fpga_id %d,Before Recv Time %16.4Lf (ms)\n",fpga_id,time_before);
	gettimeofday(&time_point[7], NULL);
	// long double time_before_recv = (time_point[7].tv_sec) * 1000.0 + (time_point[7].tv_usec) / 1000.0;
...
#ifdef DEBUG	
		fprintf(stdout, "\n*****fpga_id = %d CPU Get task data from FPGA (Addr: 0x%08lx, Data size: %d bytes).\n", fpga_id,recv_data_base, recv_data_size);
#endif
		flock(c2hfd[0], LOCK_EX);
		rc = transfer_from_fpga(DEV_C2H[fpga_id][0], c2h_fd, result, recv_data_size, recv_data_base);
		#ifdef MULTI_CARD_DEBUG
