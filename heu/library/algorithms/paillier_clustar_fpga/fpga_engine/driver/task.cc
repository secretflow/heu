 ...
 * Para         : no para
 */
void init_task(void *map_base, struct fpga_modexp_task *task,
               uint32_t task_id) {
  char *task_base = static_cast<char *>(map_base) + TASK_MANAGER_BASE_ADDR +
                    task_id * TASK_MANAGER_ADDR_SIZE;
