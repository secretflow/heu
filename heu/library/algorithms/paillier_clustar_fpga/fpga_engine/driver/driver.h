...
// #define MAX_MEMSPACE 8589672448  // 8589672448 = (1024 * 1024 * 1024 - 32 * 1024) * 8 bit 1000MB
...
typedef struct{
    ...
    size_t   data2_size;       //data size of data2. Set the parameter to 0 if there is no data2
    size_t   data3_size;       //data size of data3. Set the parameter to 0 if there is no data3
    ...
    //0 represents CPU. 128-(128+15) represent task id on FPGA. 255 repesents no data available. Same for data2, data3 and result.
    uint8_t  data2_memtype;
    uint8_t  data3_memtype;
    uint8_t  result_memtype;   //Specify the location to store result data.
    uint8_t  srcdata_buf_hold; //Specify whether to hold data in back_data_status and result_buffer registers of the source data.
 ...
 * Description : used to achieve fedai_operator accleration in FPGA, including handshake between FPGA and HOST,
 *               register configuration, data transfer, Modexp_Multifunc accleration, etc.
 * Para:
 *   task      : FPGA's task struct
 *   data      : ModexpMultifunc's data (MEN) to FPGA
 *   cipher    : ModexpMultifunc's result from FPGA
 *   data_size : byte size of ModexpMultifunc's data
 *   n_length  : bit length of ModexpMultifunc Key N^2
 *   bypass    : ModexpMultifunc's bypass mode (on: 1, off: 0)
 * Return:
 *   0         : SUCCESS
 *   other     : FAILURE, refer to Error Id.
 */
