...
#define FPGA_BUSY_FLAG          1  //FPGA is in Busy state, and encryption operation is in progress
...
#define ERROR_CLEAN_BUF         258  // Buffer clean error, the corresponding task_id is under operation or there's no buffer data on the task_id
#define ERROR_BUF_EXCEEDED      259  // There is no enough task id left to calculate buffer tasks
#define ERROR_DATA_MEMTYPE      260  // Memtypes of input data do not match the operate_mode
#define ERROR_RES_MEMTYPE       261  // Memtype of result data equals to those of input datas or not supported or corresponding task id is being used
#define ERROR_SRCDAT_BUFINFO    262  // Information buffered in FPGA contradicts cfg of input data
#define ERROR_SRCDAT_BUFSTATE   263  // Task_id for buffered source data has no data stored on it
...
#define ERROR_DATASIZE          522  // Total input data size is larger than MAX_MEMSPACE or the data size is 0
#define ERROR_REG_OPERATION     523  // Unknown problems on setting FPGA registers, unexpected interaction process
...
#define ERROR_TIMEOUT_MAX       529  // Time out detected, the longest waiting time exceeded
#define ERROR_GETBACKDATA       530  // Abnormal, Back data is not fetched, causing its status to be 1 always
#define ERROR_FPGA_SRC_FLAG     531  // Interaction error, when lauch_en is configured to 1, src_data_flag is 1;         1
#define ERROR_FPGA_BACK_FLAG    532  // Interaction error, when lauch_en is configured to 1, backdata_vld_flag is 1;     2 
#define ERROR_PISUM_MODE        533  // In Pisum mode, the Pisum_block_num or the Pisum_cfg is wrong configure
#define ERROR_PISUM_CFG         534  // In Pisum mode, when Pisum_block_num > 1 and the Pisum_cfg = 1, the batch_size is not the Integer multiple of pisum_block_num
