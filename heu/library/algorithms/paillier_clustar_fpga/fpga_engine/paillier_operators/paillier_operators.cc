...
void cpu_invert_pen_mul_fpn(void *th_para) {
	...
	for (i = para->s_index; i < para->e_index; i++) {
		...
		fpn_cal.FromMagBytes({para->fpn_encode + src_1 * para->e_length * 2 / 8, para->e_length * 2 / 8}, Endian::little);
		pen_cal.FromMagBytes({para->pen_cipher + src_2 * para->r_length / 8, para->r_length / 8}, Endian::little);

		if(fpn_cal >= keys->tmp) {
			MPInt::InvertMod(pen_cal, keys->nsquare_cal, &neg_c_cal);
			neg_scalar_cal = keys->n_cal - fpn_cal;
			CMPIntWrapper::MPIntToBytes(neg_c_cal, para->data1 + src_data * para->r_length / 8, para->r_length / 8);
			CMPIntWrapper::MPIntToBytes(neg_scalar_cal, para->data2 + src_data * para->e_length / 8, para->e_length / 8);
        }
        else{
			...
			memcpy(para->data2 + src_data * para->e_length / 8,
					para->fpn_encode + src_1 * para->e_length * 2 / 8, para->e_length / 8);
        }
...
void fpn_matrix_elementwise_multiply_pen_matrix(
  char* fpn_encode, void* fpn_base_void, void* fpn_exp_void,
  char* pen_cipher, void* pen_base_void, void* pen_exp_void,
  char* res_cipher, void* res_base_void, void* res_exp_void,
  size_t fpn_dim0, size_t fpn_dim1, size_t pen_dim0, size_t pen_dim1,
  char* n, char* g, char* nsquare, char* max_int,
  size_t encode_bitlength, size_t cipher_bitlength, size_t device_num) {
	...
	size_t batch_size_max = floor(MAX_MEMSPACE / (double)(data1_length + data2_length));
	...
	int rc = posix_memalign((void **)&para, 4096, (sizeof(char) * cfg->para_data_size) + 4096);
	if (rc != 0) {check_error_status(rc); free(cfg); return;}
	memcpy(para, nsquare, para_length / 8);

	size_t malloc_size = batch_size_res > batch_size_max ? batch_size_max : batch_size_res;
	size_t data1_alloc_size = (sizeof(char) * malloc_size * data1_length / 8) + 4096;
	rc = posix_memalign((void **)&data1, 4096, data1_alloc_size);
	if (rc != 0) {check_error_status(rc); free(cfg); free(para); return;}
	memset(data1, 0, data1_alloc_size);

	size_t data2_alloc_size = (sizeof(char) * malloc_size * data2_length / 8) + 4096;
	rc = posix_memalign((void **)&data2, 4096, data2_alloc_size);
	if (rc != 0) {check_error_status(rc); free(cfg); free(para); free(data1); return;}
	...
	for (size_t i = 0; i < batch_size_res; i += batch_size_max) {
		...
		for (size_t j = 0; j < CPU_CORE_NUM; j++) {
			auto fut = std::async(std::launch::async, cpu_invert_pen_mul_fpn, &th_para[j]);
		...
		rc = fpga_fedai_operator_accl_split_task(cfg, para, data1, data2, NULL, result);
		if (rc != 0) {check_error_status(rc); free(cfg); free(para); free(data1); free(data2); return;}
...
void pen_sum_with_same_exp (
  char* pen_cipher, void* pen_base_void, void* pen_exp_void,
  char* res_cipher, void* res_base_void, void* res_exp_void,
  size_t pen_dim0,  size_t pen_dim1,
  char* n, char* g, char* nsquare, char* max_int,
  size_t cipher_bitlength, size_t device_num) {
	...
	if (batch_size_max < pen_dim1){
		...
		char *para_extra;
		rc = posix_memalign((void **)&para_extra, 4096, (sizeof(char) * (cfg->para_data_size + 4)) + 4096); // 4 bytes are reserved for sum
		if (rc != 0) {check_error_status(rc); free(cfg); free(cfg_extra); free(para); return;}

		memcpy(para_extra, nsquare, para_length / 8);
		memcpy(cfg_extra, cfg, sizeof(fpga_config));

		char *result_sum, *pointer_swap;
		rc = posix_memalign((void **)&tmp_result, 4096, (sizeof(char) * (pen_dim0 * (size_t)ceil((double)pen_dim1 / batch_size_max) * para_length / 8)) + 4096);
		if (rc != 0) {check_error_status(rc); free(cfg); free(cfg_extra); free(para); free(para_extra); return;}
		...
		while(batch_size_max < cur_pen_dim1){
			...
			batch_size_extra = cur_pen_dim1 % batch_size_max == 0 ? batch_size_max : cur_pen_dim1 % batch_size_max;
			// printf("batch_size_max: %ld, batch_num_per_row: %ld, batch_size_extra: %ld\n", batch_size_max, batch_num_per_row , batch_size_extra);
			...
			for (i = 0; i < pen_dim0; i++){
				for (j = 0; j < batch_num_per_row; j++){
					if (j != batch_num_per_row - 1){
						rc = fpga_fedai_operator_accl_split_task(cfg, para, data1, NULL, NULL, result_sum);
						data1 = data1 + cfg->data1_size;
					}
					else{
						rc = fpga_fedai_operator_accl_split_task(cfg_extra, para_extra, data1, NULL, NULL, result_sum);
						data1 = data1 + cfg_extra->data1_size;
					}
					result_sum = result_sum + para_length / 8;
					if (rc != 0) {check_error_status(rc); free(cfg); free(cfg_extra); free(para); free(para_extra); free(tmp_result); return;}
				}
			}
	...
	for(i = 0; i < pen_dim0; i += max_row_per_batch){
		...
		rc = fpga_fedai_operator_accl_split_task(cfg, para, data1, NULL, NULL, result);
		if (rc != 0) {check_error_status(rc); free(cfg); free(para); return;}
	...
	if (tmp_result != NULL){free(tmp_result); tmp_result = NULL;}
	free(para);
	free(cfg);
}

void encrypt_without_obf(
  char* fpn_encode,  void* fpn_base_void,    void* fpn_exp_void,
  char* res_cipher, void* res_base_void, void* res_exp_void,
  char* n, char* g, char* nsquare, char* max_int,
  ...
  size_t vector_size, size_t device_num) {
	...
	fpga_config *cfg;
	cfg = (fpga_config *)malloc(sizeof(fpga_config));
	cfg->operate_mode = 15;//call encrypt without obfucation function of FPGA to calculate
	...
	rc = posix_memalign((void **)&para, 4096, (sizeof(char) * cfg->para_data_size) + 4096);
	if (rc != 0) {check_error_status(rc); free(cfg); return;}
	...
	size_t malloc_size = batch_size_res > batch_size_max ? batch_size_max : batch_size_res;
	rc = posix_memalign((void **)&data1, 4096, (sizeof(char) * malloc_size * data1_length / 8) + 4096);
	if (rc != 0) {check_error_status(rc); free(cfg); free(para); return;}
	...
	for(i = 0; i < batch_size_res; i += batch_size_max){
		...
		for(j = 0; j < batch_size_cur; j++){
			memcpy(data1 + j * data1_length / 8, fpn_encode + (i + j) * 2 * data1_length / 8, data1_length / 8);
		}
...
void obf_modular_exponentiation(char* randoms, size_t random_bitlength,
  char* n, char* g, char* nsquare, char* max_int,
...
void obf_modular_multiplication( \
  char* pen_cipher, void* pen_base_void, void* pen_exp_void, char* obf_seeds,
  char* res_cipher, void* res_base_void, void* res_exp_void,
  char* n, char* g, char* nsquare, char* max_int,
...
void decrypt(
  char* pen_cipher, void* pen_base_void, void* pen_exp_void,
  char* res_encode, void* res_base_void, void* res_exp_void,
  char* n, char* g, char* nsquare, char* max_int,
  char* p, char* q, char* psquare, char* qsquare,
  char* q_inverse, char* hp, char* hq,
  size_t encode_bitlength, size_t cipher_bitlength,
  size_t vector_size, size_t device_num) {
	...
	size_t para_length  = cipher_bitlength / 4; //special treatment in FPGA, bitlength of q
	size_t data1_length = cipher_bitlength;
	size_t data2_length = 0;
	size_t data3_length = 0;
	...
	malloc_size = batch_size_res > batch_size_max ? batch_size_max : batch_size_res;
	rc = posix_memalign((void **)&result, 4096, (sizeof(char) * malloc_size * cipher_bitlength / 8) + 4096);
	if (rc != 0) {check_error_status(rc); free(cfg); free(para); return;}
	...
	for(i = 0; i < batch_size_res; i += batch_size_max){
		...
		rc = fpga_fedai_operator_accl_split_task(cfg, para, data1, NULL, NULL, result);
		if (rc != 0) {check_error_status(rc); free(para); free(cfg); free(result); return;}
		data1 = data1 + cfg->data1_size;
		for(j = 0; j < batch_size_cur; j++){
			memcpy(res_encode + (i + j) * para_length * 4 / 8, result + j * para_length * 2 / 8, para_length * 2 / 8);
			memset(res_encode + ((2 * (i + j)) + 1) * para_length * 2 / 8, 0, para_length * 2 / 8);
		}
	}
...
void pen_add_with_same_exp (
  char* pen1_cipher, void* pen1_base_void, void* pen1_exp_void,
  char* pen2_cipher, void* pen2_base_void, void* pen2_exp_void,
  char* res_cipher,  void* res_base_void,  void* res_exp_void,
  size_t pen1_dim0, size_t pen1_dim1, size_t pen2_dim0, size_t pen2_dim1,
  char* nsquare, size_t cipher_bitlength, size_t device_num) {
	...
	rc = posix_memalign((void **)&data1, 4096, (sizeof(char) * malloc_size * data1_length / 8) + 4096);
	if (rc != 0) {check_error_status(rc); free(cfg); free(para); return;}
	rc = posix_memalign((void **)&data2, 4096, (sizeof(char) * malloc_size * data2_length / 8) + 4096);
	if (rc != 0) {check_error_status(rc); free(cfg); free(para); free(data1); return;}
	...
	for(i = 0; i < batch_size_res; i += batch_size_max) {
		...
		for (j = 0; j < batch_size_cur; j++) {
			...
			memcpy(data1 + j * data1_length / 8, pen1_cipher + src_1 * data1_length / 8, data1_length / 8);
			memcpy(data2 + j * data2_length / 8, pen2_cipher + src_2 * data2_length / 8, data2_length / 8);
			res_exp[src] = pen1_exp[src_1];
			res_base[src] = pen1_base[0];
		}

		rc = fpga_fedai_operator_accl_split_task(cfg, para, data1, data2, NULL, result);
		if (rc != 0) {check_error_status(rc); free(para); free(data1); free(data2); free(cfg); return;}
		result = result + batch_size_cur * para_length / 8;
	}
...
void cpu_encode_int(void *th_para) {
	...
	for (i = para->s_index; i < para->e_index; i++) {
        ...
        para->res_exp[i] = exp;
        if(positive == 1) {
			memcpy(para->res_encode + (uint64_t)i * para->bitlength / 8, &res_encode, sizeof(unsigned long));
		}
		else{
			res_fpga.Set(res_encode);
			res_fpga = res_fpga - keys->n_cal;
			CMPIntWrapper::MPIntToBytes(res_fpga, para->res_encode + (uint64_t)i * para->bitlength / 8, para->bitlength / 8);
		}
        para->res_base[i] = 16;
    }
}

void encode_int(void* ints_void, char* res_encode, void* res_base_void, void* res_exp_void,
				int32_t precision, char* n, char* max_int, size_t encode_bitlength,
				size_t vector_size, size_t device_num) {
...
void cpu_decode_int(void *th_para) {
	...
	for (i = para->s_index; i < para->e_index; i++) {
		numbers_fpga.FromMagBytes({para->numbers_encode + (uint64_t)i * para->bitlength / 8, para->bitlength / 8}, Endian::little);
        para->ints[i] = 0;
        positive = 1;
        ...
        if(numbers_fpga > keys->maxint_cal && numbers_fpga < keys->tmp) { //Overflow detected in decode number
        ...
        CMPIntWrapper::MPIntToBytes(numbers_fpga, res_char, para->bitlength / 8); // res_char set to 0 in wrapper
		for(j = 0; j < 1024 / 8; j++){
...
void decode_int(char* numbers_encode, void* numbers_base_void, void* numbers_exp_void,
				char* n, char* max_int, size_t encode_bitlength, void *res_void, size_t vector_size) {
	...
	keys->tmp.SetZero();
	keys->n_cal.FromMagBytes({n, encode_bitlength / 8}, Endian::little);
	keys->maxint_cal.FromMagBytes({max_int, encode_bitlength / 8}, Endian::little);
	keys->tmp = keys->n_cal - keys->maxint_cal;
...
void mpint_random(char *res, size_t plain_bitlength, size_t vec_size, char* max_num) {
	...
	for (size_t i = 0; i < vec_size; i++) {
		...
		cur_rd.ToBytes(reinterpret_cast<unsigned char*>(res + i * plain_bitlength / 8), output_byte_len, Endian::little);
	}
}
...
void check_error_status(int rc) {
    switch (rc) {
        ...
        case ERROR_CLEAN_BUF:
            raise_error("Buffer clean error. The corresponding task_id is under operation or there's no buffer data on the task_id.");
            break;
        ...
        case ERROR_RES_MEMTYPE:
            raise_error("Memtype of result data is not supported. It equals to those of input data or is being used by other processes.");
            break;
        case ERROR_SRCDAT_BUFINFO:
            raise_error("Information buffered in FPGA contradicts config of input data");
            break;
        ...
        case ERROR_RECV_DATA:
            ...
            break;
        case ERROR_DATASIZE:
            raise_error("Input data size exceeds the maximum storage space or equals 0.");
            break;
        case ERROR_REG_OPERATION:
            raise_error("Unexpected interaction process detected when setting FPGA registers' value.");
            break;
        case ERROR_OPERATEMODE:
            raise_error("Unsupported operation mode.");
            break;
        case ERROR_LENGTH:
            raise_error("Invalid bitlength of key. We currently support 128, 256, 512, 768, 1024, 2048, 3072, 4096.");
            break;
        ...
        case ERROR_TIMEOUT_CAL:
            raise_error("Time out error detected during calculation. Device reset is required.");
            break;
        case ERROR_TIMEOUT_MAX:
            raise_error("Calculation time exceeds maximum waiting time. Device reset is required.");
            break;
        case ERROR_GETBACKDATA:
            raise_error("Result data is somehow not retrieved, keeping this task id busy. Register initiation is required.");
            break;
        case ERROR_FPGA_SRC_FLAG:
            raise_error("Unexpected interaction process detected when launching the task.");
            break;
        case ERROR_FPGA_BACK_FLAG:
            raise_error("Unexpected interaction process detected when launching the task. There are other result data stored in this task id.");
            break;
        case ERROR_PISUM_MODE:
            raise_error("In Pisum mode, the Pisum_block_num or the Pisum_cfg is a wrong configure");
            break;
        case ERROR_PISUM_CFG:
            raise_error("In Pisum mode, when Pisum_block_num > 1 and the Pisum_cfg = 1, the batch_size is not the Integer multiple of pisum_block_num");
            break;
