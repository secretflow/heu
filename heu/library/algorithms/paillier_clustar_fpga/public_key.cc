...
PublicKey::PublicKey(const PublicKey& pub_key) {
    InitCopy(pub_key);
}

PublicKey::PublicKey(PublicKey&& pub_key) {
    InitMove(std::move(pub_key));
}

PublicKey::PublicKey(const MPInt& n) : n_(n) {
    Init();
}

PublicKey::PublicKey(MPInt&& n) : n_(std::move(n)) {
    Init();
}
...
bool PublicKey::operator==(const PublicKey &other) const {
    return n_ == other.n_ && g_ == other.g_ 
           && n_square_ == other.n_square_ && max_int_ == other.max_int_
           && pt_bound_ == other.pt_bound_;
}
...
std::string PublicKey::ToString() const {
    return fmt::format("clustar fpga public key: n = {}[{}bits], max_plaintext={}[~{}bits]",
...
const MPInt& PublicKey::GetN() const {
    return n_;
}

const MPInt& PublicKey::GetG() const {
    return g_;
}

const MPInt& PublicKey::GetNSquare() const {
    return n_square_;
}

const Plaintext PublicKey::GetMaxInt() const {
    return max_int_;
}
