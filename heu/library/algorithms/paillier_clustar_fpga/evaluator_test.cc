...
protected:
    ...
    template <typename T>
    ConstSpan<Plaintext> CleartextToConstSpan(std::vector<T>& input_vec, std::vector<Plaintext>& plain_vec, std::vector<Plaintext*>& plain_ptrs);

    template <typename T>
    Span<Plaintext> CleartextToSpan(std::vector<T>& input_vec, std::vector<Plaintext>& plain_vec, std::vector<Plaintext*>& plain_ptrs);

    // T: Ciphertext/Plaintext
    template <typename T>
    ConstSpan<T> TextToConstSpan(std::vector<T>& input_vec, std::vector<T*>& text_ptrs);
...
template <typename T>
ConstSpan<Plaintext> CEvaluatorTest::CleartextToConstSpan(std::vector<T>& input_vec, 
...
template <typename T>
Span<Plaintext> CEvaluatorTest::CleartextToSpan(std::vector<T>& input_vec, std::vector<Plaintext>& plain_vec, std::vector<Plaintext*>& plain_ptrs) {
    size_t vec_size = input_vec.size();
    for (size_t i = 0; i < vec_size; i++) {
        plain_vec.push_back(Plaintext(input_vec[i]));
    }
...
template <typename T>
ConstSpan<T> CEvaluatorTest::TextToConstSpan(std::vector<T>& input_vec, std::vector<T*>& text_ptrs) {
...
template <typename T>
Span<T> CEvaluatorTest::TextToSpan(std::vector<T>& input_vec, std::vector<T*>& text_ptrs) {
    CMonoFacility::ValueVecToPtrVec(input_vec, text_ptrs);
...
TEST_P(CEvaluatorTest, Randomize) {
    ...
    auto a_cipher_span = absl::MakeSpan(a_cipher_vec_ptrs.data(), a_cipher_vec_ptrs.size());

    // randomize
    evaluator_->Randomize(a_cipher_span);

    // decrypt
    auto a_const_cipher_span = absl::MakeSpan(a_cipher_vec_ptrs.data(), a_cipher_vec_ptrs.size());
    std::vector<Plaintext> res_plain_vec = decryptor_->Decrypt(a_const_cipher_span);
...
TEST_P(CEvaluatorTest, CipherAddCipher) {
    ...
    std::vector<Plaintext> a_plain_vec;
    std::vector<Plaintext*> a_plain_ptrs;
    auto a_const_plain_span = CleartextToConstSpan(a_vec, a_plain_vec, a_plain_ptrs);
    ...
    auto b_const_plain_span = CleartextToConstSpan(b_vec, b_plain_vec, b_plain_ptrs);
    ...
    // add
    std::vector<Ciphertext> res_cipher = evaluator_->Add(a_const_cipher_span, b_cipher_span);
...
TEST_P(CEvaluatorTest, CipherAddPlain) {
    ...
    // add
    std::vector<Ciphertext> res_cipher = evaluator_->Add(a_const_cipher_span, b_const_plain_span);
...
TEST_P(CEvaluatorTest, PlainAddCipher) {
    ...
    // add
    std::vector<Ciphertext> res_cipher = evaluator_->Add(a_const_plain_span, b_const_cipher_span);
...
TEST_P(CEvaluatorTest, PlainAddPlain) {
    ...
    // add
    std::vector<Plaintext> res_plain = evaluator_->Add(a_const_plain_span, b_const_plain_span);
...
TEST_P(CEvaluatorTest, CipherSubCipher) {
    ...
    // sub
    std::vector<Ciphertext> res_cipher = evaluator_->Sub(a_const_cipher_span, b_const_cipher_span);
...
TEST_P(CEvaluatorTest, CipherSubPlain) {
    ...
    // sub
    std::vector<Ciphertext> res_cipher = evaluator_->Sub(a_const_cipher_span, b_const_plain_span);
...
TEST_P(CEvaluatorTest, PlainSubCipher) {
    ...
    // sub
    std::vector<Ciphertext> res_cipher = evaluator_->Sub(a_const_plain_span, b_const_cipher_span);
...
TEST_P(CEvaluatorTest, PlainSubPlain) {
    ...
    // sub
    std::vector<Plaintext> res_plain = evaluator_->Sub(a_const_plain_span, b_const_plain_span);
...
TEST_P(CEvaluatorTest, CipherMulPlain) {
    ...
    // mul
    std::vector<Ciphertext> res_cipher = evaluator_->Mul(a_const_cipher_span, b_const_plain_span);
...
TEST_P(CEvaluatorTest, PlainMulCipher) {
    ...
    // mul
    std::vector<Ciphertext> res_cipher = evaluator_->Mul(a_const_plain_span, b_const_cipher_span);
...
TEST_P(CEvaluatorTest, PlainMulPlain) {
    ...
    // mul
    std::vector<Plaintext> res_plain = evaluator_->Mul(a_const_plain_span, b_const_plain_span);
...
TEST_P(CEvaluatorTest, PlainInplaceMulPlain) {
    ...
    // mul
    std::vector<Plaintext> res_plain = evaluator_->Mul(a_plain_span, b_const_plain_span);
...
TEST_P(CEvaluatorTest, CalcSum) {
    ...
    auto input_const_plain_span = CleartextToConstSpan(input_vec, input_plain_vec, input_plain_ptrs);
    auto input_cipher_vec = encryptor_->Encrypt(input_const_plain_span);

    std::vector<Ciphertext*> input_ciphertext_ptrs;
    auto input_const_cipher_span = TextToConstSpan(input_cipher_vec, input_ciphertext_ptrs);
