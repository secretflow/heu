...
void Evaluator::Randomize(Span<Ciphertext> ct) const {
  ...
  ConstSpan<Ciphertext> zero_ciphers_span =
      absl::MakeConstSpan(zero_ciphers_ptr.data(), zero_ciphers_ptr.size());
  AddInplace(ct, zero_ciphers_span);
}

// Use pen_add_with_same_exp instead of pen_matrix_add_pen_matrix to calc int
// only
std::vector<Ciphertext> Evaluator::Add(ConstSpan<Ciphertext> a,
  ...
  // 1-1 prepare left
  std::shared_ptr<char> left_pen(
      fpga_engine::c_malloc_init_zero(key_conf_.cipher_byte_ * elem_size),
      lambda_deleter);
  std::shared_ptr<char> left_base(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * elem_size),
      lambda_deleter);
  std::shared_ptr<char> left_exp(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * elem_size),
      lambda_deleter);
  CMonoFacility::CipherHeuToFpga(a, key_conf_.cipher_byte_, left_pen, left_base,
                                 left_exp);

  // 1-2 prepare right
  std::shared_ptr<char> right_pen(
      fpga_engine::c_malloc_init_zero(key_conf_.cipher_byte_ * elem_size),
      lambda_deleter);
  std::shared_ptr<char> right_base(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * elem_size),
      lambda_deleter);
  std::shared_ptr<char> right_exp(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * elem_size),
      lambda_deleter);
  CMonoFacility::CipherHeuToFpga(b, key_conf_.cipher_byte_, right_pen,
                                 right_base, right_exp);
  ...
  fpga_engine::pen_add_with_same_exp(
      left_pen.get(), left_base.get(), left_exp.get(), right_pen.get(),
      right_base.get(), right_exp.get(), res_pen.get(), res_base.get(),
      res_exp.get(), left_row, left_col, right_row, right_col, pub_n_square,
  ...
  CMonoFacility::CipherFpgaToHeu(res_pen, res_exp, elem_size,
...
std::vector<Ciphertext> Evaluator::Add(ConstSpan<Ciphertext> a,
  ...
  std::shared_ptr<char> right_exp(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * elem_size),
      lambda_deleter);
  CMonoFacility::CipherVecToFpga(right_cipher, key_conf_.cipher_byte_,
                                 right_pen, right_base, right_exp);
...
void Evaluator::AddInplace(Span<Ciphertext> a, ConstSpan<Ciphertext> b) const {
  YACL_ENFORCE(a.size() == b.size(),
               "AddInplace Cipher + Cipher error: size mismatch");
  size_t elem_size = a.size();
  auto result = Add(a, b);
  for (size_t i = 0; i < elem_size; i++) {
...
void Evaluator::AddInplace(Span<Ciphertext> a, ConstSpan<Plaintext> b) const {
  YACL_ENFORCE(a.size() == b.size(),
               "AddInplace Cipher + Plain error: size mismatch");
  size_t elem_size = a.size();
  auto result = Add(a, b);
  for (size_t i = 0; i < elem_size; i++) {
    *a[i] = std::move(result[i]);
...
void Evaluator::AddInplace(Span<Plaintext> a, ConstSpan<Plaintext> b) const {
  YACL_ENFORCE(a.size() == b.size(),
               "AddInplace Plain + Plain error: size mismatch");
  size_t elem_size = a.size();
  auto result = Add(a, b);
  for (size_t i = 0; i < elem_size; i++) {
    *a[i] = std::move(result[i]);
...
std::vector<Ciphertext> Evaluator::Sub(ConstSpan<Ciphertext> a,
  ...
  ConstSpan<Ciphertext> neg_b_span =
      absl::MakeConstSpan(neg_b_ptr.data(), neg_b_ptr.size());
...
void Evaluator::SubInplace(Span<Ciphertext> a, ConstSpan<Ciphertext> b) const {
  YACL_ENFORCE(a.size() == b.size(),
               "SubInplace Cipher - Cipher error: size mismatch");
  auto res = Sub(a, b);
...
void Evaluator::SubInplace(Span<Ciphertext> a, ConstSpan<Plaintext> p) const {
  YACL_ENFORCE(a.size() == p.size(),
               "SubInplace Cipher - Plain error: size mismatch");
  auto res = Sub(a, p);
  size_t vec_size = res.size();
  for (size_t i = 0; i < vec_size; i++) {
    *a[i] = res[i];
...
void Evaluator::SubInplace(Span<Plaintext> a, ConstSpan<Plaintext> b) const {
  YACL_ENFORCE(a.size() == b.size(),
               "SubInplace Plain - Plain error: size mismatch");
  auto res = Sub(a, b);
  size_t vec_size = res.size();
  for (size_t i = 0; i < vec_size; i++) {
    *a[i] = res[i];
...
std::vector<Ciphertext> Evaluator::Mul(ConstSpan<Ciphertext> a,
                                       ConstSpan<Plaintext> b) const {
  YACL_ENFORCE((a.size() == b.size()) || (a.size() == 1) || (b.size() == 1),
               "Cipher * Plain error: size mismatch");

  // Step 1 data prepration
  // 1-1 data format for cipher
  auto lambda_deleter = [](char *ptr) { free(ptr); };
  size_t a_size = a.size();
  std::shared_ptr<char> cipher_pen(
      fpga_engine::c_malloc_init_zero(key_conf_.cipher_byte_ * a_size),
      lambda_deleter);
  std::shared_ptr<char> cipher_base(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * a_size),
      lambda_deleter);
  std::shared_ptr<char> cipher_exp(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * a_size),
      lambda_deleter);
  CMonoFacility::CipherHeuToFpga(a, key_conf_.cipher_byte_, cipher_pen,
                                 cipher_base, cipher_exp);

  // 1-2 encode for plain
  size_t b_size = b.size();
  std::shared_ptr<int64_t[]> pt_arr(new int64_t[b_size]);
  CMonoFacility::PlainHeuToFpga(b, pt_arr);
  std::shared_ptr<char> pt_fpn(
      fpga_engine::c_malloc_init_zero(key_conf_.plain_byte_ * b_size),
      lambda_deleter);
  std::shared_ptr<char> pt_base(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * b_size),
      lambda_deleter);
  std::shared_ptr<char> pt_exp(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * b_size),
      lambda_deleter);
  char *pub_key_n = pub_key_helper_.GetBytesN();
  CMonoFacility::FpgaEncode(pub_key_n, b_size, key_conf_.plain_bits_, pt_arr,
                            pt_fpn, pt_base, pt_exp);
  ...
  std::shared_ptr<char> res_pen(
      fpga_engine::c_malloc_init_zero(key_conf_.cipher_byte_ * res_size),
      lambda_deleter);
  std::shared_ptr<char> res_base(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * res_size),
      lambda_deleter);
  std::shared_ptr<char> res_exp(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * res_size),
      lambda_deleter);
  ...
  fpga_engine::fpn_matrix_elementwise_multiply_pen_matrix(
      pt_fpn.get(), pt_base.get(), pt_exp.get(), cipher_pen.get(),
      cipher_base.get(), cipher_exp.get(), res_pen.get(), res_base.get(),
      res_exp.get(), 1, b_size, 1, a_size, pub_key_n, nullptr, pub_key_n_square,
      pub_key_max_int, key_conf_.plain_bits_, key_conf_.cipher_bits_,
      fpga_dev_num);

  // Step 3 format results back to heu
  std::vector<Ciphertext> res_vec;
  res_vec.reserve(res_size);
  CMonoFacility::CipherFpgaToHeu(res_pen, res_exp, res_size,
                                 key_conf_.cipher_byte_, res_vec);
...
std::vector<Ciphertext> Evaluator::Mul(ConstSpan<Plaintext> a,
                                       ConstSpan<Ciphertext> b) const {
  YACL_ENFORCE((a.size() == b.size()) || (a.size() == 1) || (b.size() == 1),
               "Plain * Cipher error: size mismatch");
  return Mul(b, a);
}
...
void Evaluator::MulInplace(Span<Ciphertext> a, ConstSpan<Plaintext> b) const {
  YACL_ENFORCE((a.size() == b.size()) || (b.size() == 1),
               "MulInplace Cipher * Plain error: size mismatch");
  auto result = Mul(a, b);
...
void Evaluator::MulInplace(Span<Plaintext> a, ConstSpan<Plaintext> b) const {
  YACL_ENFORCE((a.size() == b.size()) || (b.size() == 1),
               "MulInplace Plain * Plain error: size mismatch");
  auto result = Mul(a, b);
  size_t res_size = result.size();  // res_size supposed to == a.size()
  for (size_t i = 0; i < res_size; i++) {
    *a[i] == std::move(result[i]);
...
std::vector<Ciphertext> Evaluator::Negate(ConstSpan<Ciphertext> a) const {
  ...
  ConstSpan<Plaintext> neg_one_span =
      absl::MakeConstSpan(neg_one_ptr.data(), 1);
...
void Evaluator::CalcSum(Ciphertext *sum, ConstSpan<Ciphertext> input) const {
  ...
  std::shared_ptr<char> src_exp(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE * elem_size),
      lambda_deleter);
  CMonoFacility::CipherHeuToFpga(input, key_conf_.cipher_byte_, src_pen,
                                 src_base, src_exp);

  // 1-2 prepare result
  std::shared_ptr<char> res_pen(
      fpga_engine::c_malloc_init_zero(key_conf_.cipher_byte_), lambda_deleter);
  std::shared_ptr<char> res_base(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE),
      lambda_deleter);
  std::shared_ptr<char> res_exp(
      fpga_engine::c_malloc_init_zero(CFPGATypes::U_INT32_BYTE),
      lambda_deleter);
  ...
  char *pub_n_square = pub_key_helper_.GetBytesNSquare();
  size_t fpga_dev_num = 0;  // no effect
  fpga_engine::pen_sum_with_same_exp(
      src_pen.get(), src_base.get(), src_exp.get(), res_pen.get(),
      res_base.get(), res_exp.get(), 1, elem_size, nullptr, nullptr,
      pub_n_square, nullptr, key_conf_.cipher_bits_, fpga_dev_num);
