...
void Plaintext::ToBytes(unsigned char *buf, size_t buf_len, Endian endian) const {
    mp_int_.ToBytes(buf, buf_len, endian);                
}

size_t Plaintext::BitCount() const {
    return mp_int_.BitCount();
}
...
void Plaintext::Negate(Plaintext *z) const {  
    mp_int_.Negate(&z->mp_int_); 
}

void Plaintext::NegateInplace() {
    mp_int_.NegateInplace();
}

bool Plaintext::IsNegative() const {
    return mp_int_.IsNegative();
}

bool Plaintext::IsZero() const {
    return mp_int_.IsZero();
}

bool Plaintext::IsPositive() const {
    return mp_int_.IsPositive();
}
