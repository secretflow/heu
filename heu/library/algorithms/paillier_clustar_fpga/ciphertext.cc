...
Ciphertext::Ciphertext(char* mantissa, unsigned size, int exp) : size_(size), exp_(exp) {
    mantissa_ = static_cast<char*>(malloc(size_));
    memset(mantissa_, 0, size_); 
...
std::string Ciphertext::ToString() const {
    ...
    for (int i = 0; i < static_cast<int>(size_); i++) {
        ss<<std::hex<<std::setfill('0')<<std::setw(2)<<+static_cast<uint8_t>(*(mantissa_ + i));
    }
...
char* Ciphertext::GetMantissa() const {
    return mantissa_;
}

void Ciphertext::SetExp(int exp) {
    exp_ = exp;
}

int Ciphertext::GetExp() const {
    return exp_;
}

unsigned Ciphertext::GetSize() const {
    return size_;
}
