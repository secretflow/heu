...
private:
    ...
    // Extra helpers
    mutable bool byte_g_flag_ = false; // Indicate whether g_ has transformed to bytes
    mutable std::shared_ptr<char[]> bytes_g_;

    mutable bool byte_n_flag_ = false;  // Indicate whether n_ has transformed to bytes
    mutable std::shared_ptr<char[]> bytes_n_;

    mutable bool byte_n_square_flag_ = false;   // Indicate whether n_square has transformed to bytes
    mutable std::shared_ptr<char[]> bytes_n_square_;

    mutable bool byte_max_int_flag_;    // Indicate whether max_int_ has transformed to bytes
...
template<typename T>
void CPubKeyHelper::ToBytes(const T& input, bool& exe_flag, char* result_arr) const {
