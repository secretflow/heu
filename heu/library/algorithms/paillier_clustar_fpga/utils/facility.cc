...
void CMonoFacility::CipherHeuToFpga(ConstSpan<Ciphertext> input_span,
    ...
    for (auto item : input_span) {
        ...
        memcpy(base_ptr + i * CFPGATypes::U_INT32_BYTE, &cipher_base, CFPGATypes::U_INT32_BYTE);
        int cur_exp = item->GetExp();
        memcpy(exp_ptr + i * CFPGATypes::U_INT32_BYTE, &cur_exp, CFPGATypes::U_INT32_BYTE);
        i++;
    }
}
...
void CMonoFacility::CipherFpgaToHeu(const std::shared_ptr<char>& pen,
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        int cur_exp = 0;
        memcpy(&cur_exp, exp_ptr + i * CFPGATypes::U_INT32_BYTE, CFPGATypes::U_INT32_BYTE);
...
void CMonoFacility::PlainFpgaToHeu(const std::shared_ptr<char>& res_sptr, 
    ...
    for (size_t k = 0; k < res_size; k++) {
        ...
        int64_t val = 0;
        memcpy(&val, result_ptr + k * CFPGATypes::INT64_BYTE, CFPGATypes::INT64_BYTE);
...
void CMonoFacility::FpgaEncode(char *pub_key_n, 
                               size_t pts_size,
    ...
    fpga_engine::encode_int(pt_arr.get(), res_fpn.get(), res_base_fpn.get(), res_exp_fpn.get(), 
                            precision, pub_key_n, nullptr, plain_bits, pts_size, fpga_dev_num);
}
...
std::string CMonoFacility::CharToString(char* input_str, size_t size) {
    ...
    for (int i = 0; i < static_cast<int>(size); i++) {
        ss<<std::hex<<std::setfill('0')<<std::setw(2)<<+static_cast<uint8_t>(*(input_str + i));
    }
    return ss.str();
}
