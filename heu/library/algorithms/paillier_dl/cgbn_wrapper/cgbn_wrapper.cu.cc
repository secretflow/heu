...
static void mpint_handle_neg(mp_digit *buf, int used, MPInt* out) {
  mp_digit sign_bit = (mp_digit)1 << (sizeof(mp_digit) * 8 - 1);
  if ((buf[used - 1] & sign_bit) > 0 || out->n_.sign == MP_NEG) {
...
static void store2dev(dev_mem_t<BITS> *address, MPInt &z, bool handle_neg = true) {
  ...
  unsigned char *buf = (unsigned char *)malloc(z_size);
  z.ToBytes(buf, z_size, Endian::native);
  CUDA_CHECK(cudaMemset(address->_limbs, 0, sizeof(address->_limbs)));
  CUDA_CHECK(cudaMemcpy(address->_limbs, buf, z_size, cudaMemcpyHostToDevice));
...
static void store2host(MPInt &z, dev_mem_t<BITS> *address, bool handle_neg = true) {
  ...
  unsigned char *buf = (unsigned char *)malloc(z_size);
  CUDA_CHECK(cudaMemcpy(buf, address->_limbs,  z_size, cudaMemcpyDeviceToHost));

  int used = 0;
  buf_cal_used((mp_digit *)buf, z_size / sizeof(mp_digit), &used);
  if (handle_neg) {
    mpint_handle_neg((mp_digit *)buf, used, &z);
  }
  mp_sign sign = z.n_.sign;

  Endian endian = Endian::native;
  mp_ext_from_mag_bytes(&z.n_, buf, used * sizeof(mp_digit), endian);
  z.n_.sign = sign;
