...
ShareConverter::ShareConverter(const seal::SEALContext &context,
  ...
  YASL_ENFORCE(context_.first_parms_id() == ms_helper_->parms_id());
}

template <typename T>
void ShareConverter::DoH2A(LWECt &lwe, U8PRNG prng, T *out) const {
  YASL_ENFORCE(lwe.IsValid() && lwe.parms_id() == context_.first_parms_id());
  YASL_ENFORCE(out != nullptr);
...
void ShareConverter::DoA2H(RLWECt &rlwe, absl::Span<const T> shr) const {
  ...
  YASL_ENFORCE(!rlwe.is_ntt_form() && rlwe.size() == 2, "invalid RLWE");
  YASL_ENFORCE(shr.size() > 0 && shr.size() <= rlwe.poly_modulus_degree(),
